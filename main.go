...
func newNetwork(config neuralNetworkConfig) *neuralNetwork {
...
func makeInputandLabels(filename string) (*mat.Dense, *mat.Dense) {
	...
	inputData := make([]float64, 4*len(rawCSVData))
	labelData := make([]float64, 3*len(rawCSVData))
	var inputIndex int
	var labelIndex int
	for i, record := range rawCSVData {
		...
		for j, val := range record {
			...
			if j < 4 {
				inputData[inputIndex] = parsedVal
				inputIndex++
			} else {
				labelData[labelIndex] = parsedVal
				labelIndex++
			}
		}
	}
	inputs := mat.NewDense(len(rawCSVData), 4, inputData)
	labels := mat.NewDense(len(rawCSVData), 3, labelData)
